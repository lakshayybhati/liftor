-- ============================================================================
-- Base schema bootstrap migration
-- ============================================================================
-- This migration captures the existing public schema (tables, policies,
-- triggers, and helper functions) so fresh environments have all required
-- objects before incremental security/data migrations run.
-- It mostly matches supabase/schema.sql and is idempotent (re-runnable).
-- ============================================================================
...
-- Backwards compatible column additions for legacy food_extras instances
-- Only runs when the table already existed before this migration (legacy upgrade path)
do $$ begin
  -- Only run if table exists (it always will after CREATE above, but guards ALTER)
  if exists (select 1 from information_schema.tables where table_schema='public' and table_name='food_extras') then
